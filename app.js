if (process.env.NODE_ENV !== 'production') {
  ...
  app.get("/", (req, res) => {
    ...
    MongoClient.connect(url, function(err, db) {
      ...
      var lastlogin;
      dbo.collection("LastLogin").find({}).toArray(function(err, result) {     
        if (err) throw err;
        console.log(sortLastLogin(result));
  ...
  app.post('/process/uploadprofile', checkAuthenticated,(req, res) => {
    ...
    form.parse(req, (err, fields, files) => {
      if (err) {
        ...
        var newpath = process.env.HOME + "/Desktop/SKDEV/GitHub/ActivityProject/src/views/IMG/user_profile/" + req.user.studentID + "_" + req.user.username + ".jpeg";
  ...
  function insertProfileToDB(req, path) {
    MongoClient.connect(url, function(err, db) {
      ...
      var newvalues = { $set: {profilepath: path, profileFile: req.user.studentID + "_" + req.user.username + ".jpeg" } };
